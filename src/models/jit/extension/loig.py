...
class LoIGJiTDenoiserConfig(DenoiserConfig):
    internal_rank: int = 16


class LoIGJiT(JiT):
    config: LoIGJiTDenoiserConfig

    def __init__(self, config: LoIGJiTDenoiserConfig):
        super().__init__(config)

        # low rank internal guidance
        self.low_rank_final_layer = BottleneckFinalLayer(
            hidden_dim=config.hidden_size,
            bottleneck_dim=config.internal_rank,
    ...
    def forward(
        ...
        # normal prediction
        image_patches = self.final_layer(patches)
        ...
        # low rank prediction
        weak_pred_patches = self.low_rank_final_layer(patches)
        weak_pred_image = self.unpatchify(
            weak_pred_patches,
            height=height,
            width=width,
        )

        return pred_image, weak_pred_image

    def initialize_weights(self):
        super().initialize_weights()


class Denoiser(LoIGJiT):
    def __init__(self, config: LoIGJiTDenoiserConfig):
        nn.Module.__init__(self)
        LoIGJiT.__init__(self, config)


class LoIGJiTConfig(JiTConfig):
    denoiser: LoIGJiTDenoiserConfig = LoIGJiTDenoiserConfig()


class LoIGJiTModel(JiTModel):
    denoiser: LoIGJiT
    ...
    def generate(
        ...
        with self.progress_bar(total=num_inference_steps) as pbar:
            for i, timestep in enumerate(timesteps[:-1]):
                ...
                is_in_ig_time = (  # LoIG interval check
